...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !loading) {
      ...
      if (profile.role !== 'admin') {
        const redirectMap = { teacher: '/teacher', parent: '/parent' }
        router.replace(redirectMap[profile.role as 'teacher' | 'parent'] || '/login')
      }
    }
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-950">
      <DashboardSidebar role="admin" userName={profile.full_name} />
