...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !loading) {
      ...
      if (profile.role !== 'teacher') {
        const redirectMap = { admin: '/admin', parent: '/parent' }
        router.replace(redirectMap[profile.role as 'admin' | 'parent'] || '/login')
      }
    }
  ...
  const userInitials = profile.full_name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'T'

  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-950">
      <DashboardSidebar role="teacher" userName={profile.full_name} />
      <div className="flex-1 flex flex-col overflow-hidden">
        <DashboardHeader title="Teacher Portal" userName={profile.full_name || 'Teacher'} userInitials={userInitials} />
