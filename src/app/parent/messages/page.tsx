...
export default function MessagesPage() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('messages').insert({
        sender_id: student?.id || 'parent_id', // Should be actual parent user ID from auth
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 mb-1">Communication</h1>
        <p className="text-gray-600">Messages and updates from teachers</p>
      </div>
      ...
      {groups.length > 0 ? (
          ...
          <div className="lg:col-span-1">
            <Card className="border-amber-200 h-full overflow-hidden flex flex-col">
              <div className="p-4 bg-gradient-to-r from-amber-100 to-orange-100 border-b border-amber-200">
                <h2 className="font-bold text-gray-900">Class Groups</h2>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {getMemberCount(selectedGroup)} members in group
                    </p>
                  </div>
        ...
        <EmptyState
          ...
          description="You're not part of any chat groups yet. Contact the school to join."
        />
      )}
    </div>
  );
